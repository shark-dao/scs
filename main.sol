...
interface FARM_Interface {
    ...
    function userAmount(uint _pid, address _user) external view returns (uint);
}
...
interface DATA_Interface {
    function getData() external view returns (uint totalSupply, uint price_18, uint tvl_36, uint emission, uint apr);
}
...
contract Data {
    ...
    uint ethwei = 10**18;
    address SHARK = 0xB5bB9D146935ed8f10d1AE1038408dAEb68C98E6;
    address FARM = 0xf17b1B23e96F99301f638a7B8cDd64CF2fFccA10;
    address LP = 0x5Fe8B163e5b238DeC2d455093bB1F195caC5E6c6;
    
    SHARK_Interface public shark = SHARK_Interface(SHARK);
    FARM_Interface public farm = FARM_Interface(FARM);
    LP_Interface public lp = LP_Interface(LP);
    
    function getRewardShare_18(address _user) public view returns (uint) {
        uint reward = farm.userAmount(0, _user).mul(100).mul(ethwei).div(lp.balanceOf(FARM));
        return reward;
    }
    
    function getData() public view returns (uint totalSupply, uint price_18, uint tvl_36, uint emission, uint apr) {
        
        totalSupply = shark.totalSupply();
        
        (uint r0, uint r1, ) = lp.getReserves();
        price_18 = r1.mul(ethwei).div(r0);
        
        uint liquidityPerLP = (shark.balanceOf(LP).mul(price_18).mul(2)) / lp.totalSupply();
        tvl_36 = liquidityPerLP.mul(lp.balanceOf(FARM));
        
        emission = farm.getEmission();
        
        apr = emission.mul(price_18).mul(31536000).mul(100).div(tvl_36);
    }
}
...
contract TreasuryMinting is Ownable {
    ...
    address DATA = 0x9214051E51280F3D17196BbD5D6Ffb6DE811528F;
    DATA_Interface public data = DATA_Interface(DATA);
    
    address MIM = 0x58E05255baEAb9770038E3cA6488488eAFDC3ba5;
    ...
    require(_duration == 60 || _duration == 120 || _duration == 300, "Wrong time.");
       
        (,uint sharkPrice,,,) = data.getData();
        uint usdAmount = _sharkAmount.mul(1e18).div(sharkPrice);
    ...
    function claim(uint _id) public {
        require(mints[_id].owner == msg.sender, "You can't claim this mint.");

        if (block.timestamp >= mints[_id].endTime) {
            shark.transfer(msg.sender, mints[_id].sharkRemaining);
            mints[_id].sharkRemaining = 0;
            mints[_id].lastClaim = block.timestamp;
        } else {
            uint time = block.timestamp - mints[_id].lastClaim;
            uint timeLeft = mints[_id].endTime - mints[_id].lastClaim;
            uint claimAmount = (time * mints[_id].sharkRemaining) / timeLeft;

            shark.transfer(msg.sender, claimAmount);
            mints[_id].sharkRemaining -= claimAmount;
            mints[_id].lastClaim = block.timestamp;
        }
    }
}
